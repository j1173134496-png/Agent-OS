...
describe('agentosReasoningSchema', () => {
  it('accepts the verified V0.3.1 model capability contract', () => {
    const result = interfaceSchema.safeParse({
      agentosReasoning: {
        ...
        models: {
          'gpt-5.6-sol': { levels: ['low', 'medium', 'high'] },
          'gpt-5.6-terra': { levels: ['low', 'medium', 'high'] },
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.agentosReasoning?.models['gpt-5.5']?.levels).toEqual([
        'low',
        'medium',
        'high',
      ]);
  ...
  it('rejects levels outside the supported reasoning contract', () => {
    const result = interfaceSchema.safeParse({
      agentosReasoning: {
        enabled: true,
        levels: ['xhigh'],
  ...
  it('does not invent model capabilities when the matrix is absent', () => {
    const result = interfaceSchema.safeParse({
      agentosReasoning: { enabled: true },
    });

    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.agentosReasoning?.models).toEqual({});
    }
  });
});
