...
describe('loadAgent', () => {
  ...
  test('should load the native image generation tool for an ephemeral agent', async () => {
    const { EPHEMERAL_AGENT_ID } = Constants;
    const result = await loadAgent(
      {
        req: {
          user: { id: 'user123' },
          body: { ephemeralAgent: { image_gen_oai: true } },
        },
        agent_id: EPHEMERAL_AGENT_ID as string,
        endpoint: 'openai',
        model_parameters: { model: 'gpt-5.5' } as unknown as AgentModelParameters,
      },
      deps,
    );

    expect(result?.tools).toEqual(['image_gen_oai']);
  });
