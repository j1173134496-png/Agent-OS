...
async function openAgentChat(page: Page, agentId: string, agentName: string) {
  // Business Agents use their administrator-defined model and therefore enter
  // chat through the native Agent route instead of the employee model menu.
  await page.goto(`${NEW_CHAT_PATH}?agent_id=${encodeURIComponent(agentId)}`, { timeout: 10000 });
  await expect(page.getByRole('main')).toContainText(agentName, { timeout: 15000 });
...
test.describe('soft default model spec', () => {
  ...
  test('a previously selected agent outranks the soft default', async ({ page }) => {
    ...
    await openAgentChat(page, agent.id, agentName);
    ...
    const [agentRequest, response] = await Promise.all([
      ...
      sendMessage(page, `hello ${agentName}`),
    ]);
    expect(response.ok()).toBeTruthy();
    await expect(mockReply(page)).toBeVisible({ timeout: 30000 });
    expect(agentRequest.postDataJSON()).toMatchObject({ agent_id: agent.id });
  });
