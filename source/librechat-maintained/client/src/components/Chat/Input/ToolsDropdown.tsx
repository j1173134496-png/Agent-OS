...
import { Check, Globe, ImagePlus, ScrollText, Settings, Settings2, TerminalSquareIcon } from 'lucide-react';
...
const ToolsDropdown = ({ disabled }: ToolsDropdownProps) => {
  ...
  const { toggleState: isImageGenerationEnabled, debouncedChange: changeImageGeneration } =
    imageGeneration ?? {};
  ...
  const handleImageGenerationToggle = useCallback(() => {
    changeImageGeneration?.({ value: !isImageGenerationEnabled });
  }, [changeImageGeneration, isImageGenerationEnabled]);
  ...
  dropdownItems.push({
    ...
    hideOnClick: false,
    render: (props) => (
      <div {...props} data-testid="tools-menu-image-generation">
        <div className="flex items-center gap-2">
          <ImagePlus className="icon-md" aria-hidden="true" />
          <span>AI 生图</span>
        </div>
        {isImageGenerationEnabled && <Check className="size-4" aria-label="已启用" />}
      </div>
    ),
  });
