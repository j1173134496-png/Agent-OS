...
export default function BadgeRowProvider({
  ...
  useEffect(() => {
    ...
    if (!hasInitializedRef.current || lastContextKeyRef.current !== storageSuffix) {
      ...
      const imageGenerationToggleKey = `${LocalStorageKeys.LAST_IMAGE_GEN_TOGGLE_}${storageSuffix}`;
      ...
      const imageGenerationToggleValue = getTimestampedValue(imageGenerationToggleKey);
      ...
      if (imageGenerationToggleValue !== null) {
        try {
          initialValues['image_gen_oai'] = JSON.parse(imageGenerationToggleValue);
        } catch (e) {
