...
function createOpenAIImageTools(fields = {}) {
  ...
  const getApiKey = () => {
    ...
    if (!apiKey && !override) {
      throw new Error('Missing IMAGE_GEN_OAI_API_KEY or AGENTOS_LLM_API_KEY environment variable.');
    }
    return apiKey;
  };
  ...
  const configuredApiKey = fields.IMAGE_GEN_OAI_API_KEY || fields.AGENTOS_LLM_API_KEY;
  let apiKey = configuredApiKey || getApiKey();
  ...
  if (!override && configuredBaseURL) {
    const extractedBaseURL = extractBaseURL(configuredBaseURL);
    baseURL = /\/v1(?:\/|$)/.test(extractedBaseURL)
      ? extractedBaseURL.replace(/\/$/, '')
      : `${extractedBaseURL.replace(/\/$/, '')}/v1`;
