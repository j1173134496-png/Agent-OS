  ...
  async function runSummarizationLoop({ res, collectedUsage, contextUsageSink, usageEmitSink }) {
    ...
    const run = await Run.create({
      ...
      graphConfig: {
        ...
        // The installed Agents runtime creates the summarization model
        // independently from Graph.overrideModel. Use an invalid provider so
        // this unit test exercises the deterministic metadata fallback without
        // making a real network request.
        summarizationConfig: { provider: 'agentos-test-no-network' },
      },
      returnContent: true,
      customHandlers: handlers,
      tokenCounter: charCounter,
